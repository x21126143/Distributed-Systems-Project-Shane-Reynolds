...
public class MathClient {
	...
	/*
	 * //blocking server-streaming public static void
	 * generateRandomNumbersBlocking() { RandomRequest request =
	 * RandomRequest.newBuilder() .setNumbers(5).setMin(0).setMax(100).build();
	 * 
	 * try { Iterator<NumberResponse> responces =
	 * blockingStub.generateRandomNumbers(request);
	 * 
	 * while(responces.hasNext()) { NumberResponse temp = responces.next();
	 ...
	 * 
	 * 
	 * StreamObserver<NumberResponse> responseObserver = new
	 * StreamObserver<NumberResponse>() {
	 * 
	 * int count =0 ;
	 * 
	 * @Override public void onNext(NumberResponse value) {
	 * System.out.println("receiving random num " + value.getNumber()); count += 1;
	 * }
	 * 
	 ...
	 * 
	 * 
	 * StreamObserver<ConvertResponse> responseObserver = new
	 * StreamObserver<ConvertResponse>() {
	 * 
	 * int count =0 ;
	 * 
	 * @Override public void onNext(ConvertResponse msg) {
	 * System.out.println("receiving converted number " + msg.getNumber() +
	 * " base: "+ msg.getBase() ); count += 1; }
	 * 
	 * @Override public void onError(Throwable t) { t.printStackTrace();
	 * 
	 * }
	...
	/*
	 * StreamObserver<ConvertMessage> requestObserver =
	 * asyncStub.convertBase(responseObserver);
	 * 
	 * try {
	 * 
	 * requestObserver.onNext(ConvertMessage.newBuilder().setNumber("100").
	 * setFromBase(10).setToBase(2).build());
	 * requestObserver.onNext(ConvertMessage.newBuilder().setNumber("177").
	 * setFromBase(8).setToBase(10).build());
	 * requestObserver.onNext(ConvertMessage.newBuilder().setNumber("10100010").
	 * setFromBase(2).setToBase(10).build());
	 * requestObserver.onNext(ConvertMessage.newBuilder().setNumber("AF45").
	 * setFromBase(16).setToBase(2).build());
	 * requestObserver.onNext(ConvertMessage.newBuilder().setNumber("AF45").
	 * setFromBase(16).setToBase(10).build());
	 * 
	 * 
	 * // Mark the end of requests requestObserver.onCompleted();
	 * 
	 * 
	 * // Sleep for a bit before sending the next one. Thread.sleep(new
	 * Random().nextInt(1000) + 500);
	 * 
	 * 
	 * } catch (RuntimeException e) { e.printStackTrace(); } catch
